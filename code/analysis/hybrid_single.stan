...
data {
    ...
    int<lower=1, upper=2> action1[num_trials]; // First stage actions
    int<lower=1, upper=2> action2[num_trials]; // Second stage actions
    int<lower=1, upper=2> s2[num_trials]; // Second stage states
    int<lower=0, upper=1> reward[num_trials]; // Rewards
}
parameters {
